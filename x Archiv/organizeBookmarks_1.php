...
<script>
  ...
  function loadBookmarks() {
    ...
    .done(function(bookmarksJson) {
      ...
      for(b = 0; b < looD.bookmarks.length; b++) {
        var bm = looD.bookmarks[b];
        //log(bm);
        var fn = bm.href;
        //log(fn);
        if(fn.indexOf("//")>0) fn = fn.slice(fn.indexOf("//")+2);
        if(fn.indexOf("www.")>=0) fn = fn.slice(fn.indexOf("www.")+4);
        if(fn.indexOf("/")>=0) fn = fn.slice(0, fn.indexOf("/"));
        log(fn);
        if(bm.icon && fn.length>=3) {
          //$.post("../php/saveSafeCentral.php", {code: localStorage.looopCode, task: "createImage", path: "img/favicons/"+fn+".png", content: bm.icon});
          delete looD.bookmarks[b].icon;
          delete looD.bookmarks[b].iconurl;
        }
        else {
          bm.iconpath = "";
        }
      ...
      log(looD.bookmarks);
      $.post("../php/saveSafeCentral.php", {code: localStorage.looopCode, task: "save", path: "cms/bookmarks.json", content: JSON.stringify(looD.bookmarks)});
    });
  }
//  
//  function verify() {
...
////    $.getJSON("../cms/bookmarksImport.json")
////    .done(function(bookmarksJson) {
////      looD.bookmarks = bookmarksJson;
////      looD.newBookmarks = [];
////      log(bookmarksJson);
...
////      $.post("../php/saveSafeCentral.php", {code: localStorage.looopCode, path: "cms/bookmarks.json", task: "save", content: JSON.stringify(looD.newBookmarks)}, responseAnalyzer);
////    });
////  }
////  
...
//    "<td class='preview'><img src='{{b.icon}}' style='width:16px; height:16px;'></td>"+
...
//    "<td>{{getDateFromDOb(new Date(b.mod*1000))}}</td>" + // "<td>{{getDateFromDOb(new Date(b.add*1000))}}</td>" +
...
//  //log(looD.bookmarks);
//  $.post("../php/saveSafeCentral.php", {code: localStorage.looopCode, task: "save", path: "cms/bookmarks.json", content: JSON.stringify(looD.bookmarks)}, createFileTable);
//}
