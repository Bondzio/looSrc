function Term(termstring, $renderDom, $resultDom){
  ...
  this.$resultDom = $resultDom || false;
  this.$evalDom = this.$renderDom.closest("l-eq").find(".eq__eval");
  this.scope = this.$resultDom && this.$resultDom.closest("l-ex").length ? this.$resultDom.closest("l-ex")[0].scope : {}
...
Term.prototype.update = function(options) {
  ...
  this.$renderDom.empty().append(this.renderedHtml);
  //this.setPositions();
  ...
  if(this.$resultDom) {
    ...
    this.measuringNode.remove();
    this.$evalDom.empty().append(this.evalHtml);
  ...
  if(token.type === "variable") {
    ...
    for(var i = 0; i < token.modifiers.length; i++) {
      ...
      if(mod === "vec") {
        ...
        var ttt = new Term(token.modifiers[i].val, $(".null"), $(".null"));
        temp += "<span class='mod of outer'>(<span class='of inner'>"+ttt.renderedHtml+"</span>)</span>";
      }
    }
...
//      if(n.overlap.top > 0) {this.specialPositions.push({path: route + ".fracDown ", key: "top", value: fracReducer * n.overlap.top});}
    ...
    if(token.name === "pow") {
      ...
      var width = argNodes[0].w + expMargin + expReducer * argNodes[1].w;
...
Term.prototype.eval = function($el, scopeObj) {
  if($el==null) {return;}
  var term = this;
  ...
  if(currentNodes.length) {
    ...
    var resultstring = result < 0?("<span class='operator__sign minus' style='width:"+this.rOpt.signWidth+"em;'><svg height='1em' width='"+this.rOpt.signWidth+"em' viewBox='0 0 "+this.rOpt.signWidth+" 1' style='stroke:currentColor;stroke-width:"+this.rOpt.strokeThickness+"em'><line x1='0' y1='"+this.rOpt.signHeight+"' x2='"+this.rOpt.signWidth+"' y2='"+this.rOpt.signHeight+"'></line></svg></span>"):"";
    result = Math.abs(result);
    $el.empty().append(resultstring + result);
  }
  else {$el.empty();}
};
...
Term.prototype.evalInScope = function(token) {
  if(this.scope[token.raw] !== undefined) {
    var subtree = new Term("(" + this.scope[token.raw].val + "'[" + this.scope[token.raw].unit + "])", $("<div></div>"))
    return {val: this.scope[token.raw].val, unit: this.scope[token.raw].val || 1, subtree: subtree.tree[0]};
  }
